export default
  welcome:
    title1: '什么是 Fine'
    quote1: 'Fine 是帮助你在无服务器的环境下快速生成文档站点的工具，你可以用它为自己的项目快速生成文档而不用关心样式、部署、页面的问题。'
    quote2: '甚至你可以用 Fine 来生成自己的博客，Fine 为任何一个项目都提供了独特的域名。'
    quote3: '当然，最重要的是，Fine 是免费的。'

    title2: '怎么使用 Fine ?'
    usage1: '首先你需要准备一个包含 markdown 文件 (.md) 的项目，fine 的工作是帮助你把 markdown 文件转化成站点并部署它们。'
    usage2: '切换到你的项目目录下，输入以下命令：'
    usage3: '没有了？'
    usage4: '是的，很酷吧？ `npx` 允许你临时使用一个包，完成会立刻清除。你可以用这条命令试试 fine 是如何帮你生成一个文档站点的。'
    usage5: '如果你需要使用 fine，建议安装 fine 到全局: '

    title3: '使用指南'
    guide1: '你可以选择快速部署模式在几秒内生成一个站点，当然也可以让 fine.sh/cli 在项目目录下生成一个配置文件，这样你就可以拥有一个特别的域名。'
